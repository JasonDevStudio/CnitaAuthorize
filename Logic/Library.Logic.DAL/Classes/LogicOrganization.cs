...
namespace Library.Logic.Classes
{
    public class LogicOrganization 
    {
        ...
        /// <returns>执行结果</returns>
        public int OrganizationUpdateStatus(out string resultMsg,string authorizeInfo,Int32 orgid ,DbTransaction tran=null)
        {
            resultMsg = string.Empty;
            int res = 0;
            try
        ...
        /// <param name="orgid">Orgid 编号</param>
        /// <returns>执行结果</returns>
        public int OrganizationDelete(out string resultMsg, string authorizeInfo, Int32 orgid, DbTransaction tran = null)
        {
            resultMsg = string.Empty;
            int res = 0;
            try
        ...
        ///  查询所有实体
        /// </summary>  
        public IList<ModelOrganization> OrganizationAll(out string resultMsg,string authorizeInfo)
        {
            resultMsg = string.Empty;
            IList<ModelOrganization> list = new List<ModelOrganization>(); 
            try
            {
                ...
                //查询执行
                using (IDataReader dr = DBHelper.ExecuteReader(authorizeInfo,sql))
                {
                   list = GetModel(dr); 
                }
            }
